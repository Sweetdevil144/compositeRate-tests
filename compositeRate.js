...
describe('Controller Interaction', () => {
    it('should switch between controllers based on transaction count', async () => {
        const testMessage = new TestMessage('test', [], {
            weights: [0.5, 0.5],
            rateControllers: ['fixed-rate', 'block-rate']
        });
        const controller = new CompositeRateController(testMessage, {}, 0);
        await controller.applyRateControl();
        expect(controller.activeControllerIndex).to.change;
    });
});
